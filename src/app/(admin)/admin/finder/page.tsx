...
export default function AdminFinderPage() {
  ...
  // 1. Fungsi mencari film ke API internal router (Nanti kita hubungkan ke lib/tmdb)
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Kita panggil API route bawaan Next.js yang akan kita buat setelah ini
      const res = await fetch(`/api/tmdb-search?query=${searchQuery}&type=${contentType}`)
  ...
  // 2. Fungsi submit data ke Postgres menggunakan Server Actions
              ...
              {contentType === 'movie' ? (
                ...
                <div>
                    ...
                    <span className="text-[11px] text-gray-400 block mb-2">
                        💡 <strong>Tips Pasifik:</strong> Kamu bisa langsung copas baris link urut (Otomatis jadi Eps 1, 2, 3), ATAU gunakan format manual <code>nomor_eps|link_streaming</code> jika lompat-lompat.
                    </span>
                    <textarea 
                        rows={6}
                        required
                        placeholder={`https://drive.google.com/file/d/Eps1/preview\nhttps://drive.google.com/file/d/Eps2/preview\nhttps://drive.google.com/file/d/Eps3/preview\n\n(Atau format manual:)\n1|https://link.com/ep1.m3u8`}
                        value={bulkText}
                ...
                /* INPUT JIKA SERIES (BULK EPISODE) */
